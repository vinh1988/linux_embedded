...
SENSOR_DATA_PATTERN2 = re.compile(r'Sending data: DATA (\d+) ([\d\.]+) ([\d\.]+) ([\d\.]+)')
...
def parse_log_file(log_file):
    ...
    try:
        with open(log_file, 'r') as f:
            for line in f:
                # Check if line contains sensor data (server output format)
                ...
                # Check if line contains sensor data (simulator output format)
                sensor_match2 = SENSOR_DATA_PATTERN2.search(line)
                if sensor_match2:
                    node_id = int(sensor_match2.group(1))
                    temperature = float(sensor_match2.group(2))
                    humidity = float(sensor_match2.group(3))
                    pressure = float(sensor_match2.group(4))
...
def find_log_files(directory='.'):
    ...
    # Find sensor simulator log files
    for file in os.listdir(directory):
        if file.startswith('sensor_') and file.endswith('.log'):
            log_files.append(os.path.join(directory, file))
...
def parse_all_logs(directory='.'):
    ...
    for log_file in log_files:
        ...
        if data:
            print(f"Extracted {len(data)} data points from {log_file}")
...
def generate_json(data, output_file='sensor_history.json'):
    ...
    print(f"Sensor data saved to {output_file} ({len(data)} data points)")
